...
import { 
  ...
  Upload, Sparkles, AlertCircle, Loader2, ArrowRight
} from "lucide-react";
...
export default function Subscription() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!receiptUrl.trim()) {
      toast({ variant: "destructive", description: "Please provide a receipt URL." });
      return;
    }
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto animate-in fade-in duration-500 pb-10">
      ...
      <div className="text-center space-y-4 mb-12">
        ...
        <h1 className="text-4xl font-bold tracking-tight">Unlock Norv_ai Pro</h1>
        <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
          Get unlimited access to AI chatbot, advanced code debugging, custom quizzes, and personalized career advice.
        </p>
      </div>

      {statusLoading ? (
        <Card className="h-40 flex items-center justify-center border-border/50"><Loader2 className="animate-spin text-muted-foreground" /></Card>
      ) : status?.active ? (
          ...
          <CardContent className="space-y-2 relative z-10">
            ...
            <div className="flex gap-8 mt-6">
              ...
              <div>
                ...
                <p className="font-bold text-xl">{status.expiryDate ? new Date(status.expiryDate).toLocaleDateString() : "Lifetime"}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-yellow-500/50 bg-yellow-500/5 shadow-sm overflow-hidden relative">
          ...
          <CardContent className="space-y-4 relative z-10">
            <p className="text-lg">We're verifying your payment. Your account will be upgraded shortly.</p>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {PLANS.map((plan) => (
              ...
              >
                ...
                <div className="absolute bottom-4 inset-x-0 flex justify-center opacity-0 transition-opacity" style={{ opacity: selectedPlan === plan.id ? 1 : 0 }}>
            ...
            <div className="grid md:grid-cols-2">
              <div className="bg-sidebar p-8 text-sidebar-foreground flex flex-col justify-center">
                ...
                <p className="text-sidebar-foreground/80 mb-6 leading-relaxed">
                  We currently support local transfers via CliQ. Please follow these steps to activate your subscription:
                </p>
                <ol className="space-y-4">
                  ...
                  <li className="flex gap-3 items-start">
                    ...
                    <span>Take a screenshot of the successful transfer receipt and paste the link here.</span>
                  </li>
                </ol>
              </div>
              ...
              <div className="p-8 flex flex-col justify-center bg-card">
                <form onSubmit={handleSubmit} className="space-y-6 max-w-sm w-full mx-auto">
                  <div className="text-center mb-6">
                    <h3 className="text-xl font-bold mb-2">Submit Receipt</h3>
                    <p className="text-sm text-muted-foreground">Upload your receipt somewhere (like Imgur) and paste the link below.</p>
                  </div>
