...
async function ownedDeck(userId: number, deckId: number) {
  const [deck] = await db.select().from(memorizationDecksTable)
...
router.get("/memorization/decks/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  const progress = await db.select().from(flashcardProgressTable)
...
router.post("/memorization/decks/generate", requireAuth, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const [deck] = await db.insert(memorizationDecksTable).values({
      userId,
...
router.post("/memorization/decks/:id/cards/:cardId/review", requireAuth, async (req, res): Promise<void> => {
  ...
  const [current] = await db.select().from(flashcardProgressTable).where(and(
  ...
  const [progress] = await db.insert(flashcardProgressTable).values({
